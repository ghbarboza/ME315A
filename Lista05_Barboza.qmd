...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES * 1.2 AS NOVA_AREA
FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES / 3.0 AS AREA_AJUSTADA
FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE, (FEE + 50) / 2 AS TAXA_AJUSTADA
FROM PRESERVE
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT FEE) AS N_TAXAS_DISTINTAS
FROM PRESERVE
WHERE FEE <> 0
```
...
```{sql connection=con}
SELECT COUNT(PNAME) AS TOTAL, COUNT(DISTINCT PNAME) AS DISTINTOS
FROM PRESERVE
```
...
```{sql connection=con}
SELECT AVG(ACRES * 0.02) AS NOVA_TAXA_MEDIA
FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT DNO) AS N_DEPARTAMENTOS
FROM EMPLOYEE
```
